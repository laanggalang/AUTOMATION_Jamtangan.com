...
class LoginWithGoogle {
	...
	def userPilihMetodeLogin() {
		WebUI.click(new TestObject().addProperty('label', ConditionType.EQUALS, 'Google'))
		WebUI.click(new TestObject().addProperty('xpath', ConditionType.EQUALS, "(//div[@role='link'])[2]"))
		WebUI.closeBrowser()
	}

	@Then("user verify berhasil diarahkan ke halaman Homepage")
	def userVerifyBerhasilLogin() {
		println status
	}
}